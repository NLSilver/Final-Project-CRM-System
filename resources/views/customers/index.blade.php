        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full md:w-auto">
            ...
            <form action="{{ route('customers.index') }}" method="GET" id="searchForm" class="relative group w-full sm:w-64">
                <input type="text" 
                       ...
                       autocomplete="off"
                       class="pl-8 pr-4 py-1.5 border-none focus:ring-0 text-sm text-gray-600 placeholder-gray-400 w-full bg-transparent transition-all border-b border-transparent focus:border-gray-200">
                ...
                <table class="min-w-full border-collapse">
                    <thead>
                        <tr class="text-gray-400 text-[11px] uppercase border-b border-gray-100 bg-gray-50/50">
                            ...
                            <th class="whitespace-nowrap px-4 py-3 text-left font-semibold border-r border-gray-100">Contact No</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($customers->get($status, []) as $customer)
                            <tr class="hover:bg-blue-50/20 transition group">
                                <td class="px-4 py-3 text-sm font-medium text-gray-700 border-r border-gray-100 border-l-4 border-l-{{ $color }}-500">
                                    <div class="truncate max-w-[150px]">{{ $customer->first_name }} {{ $customer->last_name }}</div>
                                </td>

                                <td class="px-4 py-3 text-sm text-blue-500 border-r border-gray-100">
                                    <div class="truncate max-w-[200px] text-blue-600 font-medium">{{ $customer->email }}</div>
                                </td>

                                <td class="px-4 py-3 text-sm text-gray-600 border-r border-gray-100 whitespace-nowrap">
                                    {{ $customer->phone ?? 'N/A' }}
                                </td>

                                <td class="px-4 py-3 border-r border-gray-100">
                                    <span class="bg-gray-100 text-gray-700 text-[10px] px-2 py-1 rounded uppercase font-bold block w-fit truncate max-w-[120px]">
                                ...
                                @if(auth()->user()->role !== 'sales_staff')
                                    <td class="px-4 py-3 border-r border-gray-100">
                                        <div class="text-sm text-gray-700 truncate max-w-[120px]">{{ $customer->user->name ?? 'Unassigned' }}</div>
