...
<x-app-layout>
    ...
    <div class="bg-white shadow-sm border border-gray-200 rounded-sm overflow-hidden">
                        ...
                        <tr class="hover:bg-gray-50/50 transition group">
                            ...
                            @if(auth()->user()->role !== 'sales_staff')
                                <td class="px-4 py-3 border-r border-gray-100 text-sm text-gray-700">
                                    <div class="truncate max-w-[120px]">{{ $lead->user->name ?? 'Unassigned' }}</div>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-3">
                                    <a href="{{ route('leads.show', $lead) }}" title="View Detail">
                                        <svg class="w-4 h-4 text-gray-400 hover:text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if($lead->status !== 'Won' && $lead->status !== 'Lost')
                                        ...
                                        <form action="{{ route('leads.convert', $lead) }}" method="POST" class="inline" onsubmit="return confirm('Convert to Customer?')">
                                            @csrf
                                            <button type="submit" title="Convert to Customer">
                                    ...
                                    <form action="{{ route('leads.destroy', $lead) }}" method="POST" class="inline" onsubmit="return confirm('Delete this lead?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" title="Delete Lead">
        ...
        <div class="mt-4 px-4 py-3 bg-gray-50 border-t border-gray-100">
