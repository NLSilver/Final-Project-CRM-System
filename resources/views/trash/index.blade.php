...
<x-app-layout>
    <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        <div class="mb-8 flex justify-between items-start">
            ...
            <a href="{{ route('customers.index') }}" class="px-4 py-2 bg-white border border-gray-200 text-xs font-bold text-gray-600 rounded-lg hover:bg-gray-50 transition shadow-sm flex items-center">
                <svg class="w-3 h-3 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                EXIT ARCHIVE
            </a>
        </div>

        <div class="flex space-x-1 mb-6 bg-gray-100 p-1 rounded-xl w-fit">
            @foreach($tabs as $key => $tab)
                <a href="{{ route('trash.index', ['type' => $key]) }}" 
                   class="px-6 py-2 text-xs font-bold uppercase tracking-widest rounded-lg transition {{ $activeType === $key ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700' }}">
                    {{ $tab['label'] }}
                </a>
            @endforeach
        </div>
    ...
    <table class="min-w-full divide-y divide-gray-100">
        <thead class="bg-gray-50/50">
            <tr class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                <th class="px-6 py-4 text-left">Record Information</th>
                <th class="px-6 py-4 text-left">Deletion Metadata</th>
                <th class="px-6 py-4 text-center">Recovery Actions</th>
            </tr>
        </thead>
                ...
                <tr class="hover:bg-blue-50/20 transition-colors duration-200 group text-gray-400">
                    <td class="px-6 py-4 border-r border-gray-100">
                        <div class="flex items-center">
                            <div class="w-2 h-2 rounded-full mr-3 bg-{{ $tabs[$activeType]['color'] }}-500 shadow-sm transition-transform group-hover:scale-125"></div>
                            <div>
                                ...
                                @else
                                    ...
                                    <p class="text-[10px] font-medium italic tracking-tight">
                    ...
                    <td class="px-6 py-4 text-sm text-center">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <a href="{{ route($activeType . '.show', $item->id) }}" class="p-1 group-hover:text-blue-600 transition-colors duration-200">
